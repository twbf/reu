...
def psi(s,lambd):  #height
    result = integrate.quad(lambda k: wave_plus(k,lambd)*special.jv(1/m,2*k*np.sqrt(s)), 0, 100) #cuts at 20
    return s**(-1/(2*m))*result[0]

def phi(s,lambd):  #velocity
    result = integrate.quad(lambda k: wave_minus(k,lambd)*special.jv(1/m+1,2*k*np.sqrt(s)), 0, 100) #cuts at 20
...
for c in range(ndt):
    for i in range(ndx):
        ...
        #x1[c][i] = psi(i*dx + 1,c*dt+ 1)  # +0.001 is becasue solution breaks down at 0
    plt.plot(x[c])
    plt.plot(x1[c])
