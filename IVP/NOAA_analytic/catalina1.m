function [eta_sl, u_sl] = catalina1(save)
    ...
    upX = 10;                    % upper bound on xi
    upO = 10;                    % upper bound on omega
    upL = 1;                      % upper bound on lambda
    upS = 10;                     % upper bound on sigma
   ...
   eta_sigma0_prime = chebfun(@(sigma) -4*H1*sigma*(c1*(sigma^2-sigma1^2))/...
   256*exp(-(c1*(sigma^2-sigma1^2).^2)/256)+4*H2*sigma*(c2*(sigma^2-...
   sigma2^2))/256*exp(-(c2*(sigma^2-sigma2^2).^2)/256 ), [0 upS]);
   ...
   in_int_term2 = chebfun2(@(lambda, sigma) sum(j1(omega*sigma)/sigma*...
   j1(omega*xi)*sine(omega*lambda)), [0 upL 0.001 upS], 'vectorize');
   % Evaluates 'u' in equation (4)
   disp('u...')
   u_sl = chebfun2( @(sigma, lambda) sum(xi.^2*Phi(xi)*in_int_term2...
   (lambda, sigma)), [0.001 upS 0 upL], 'vectorize');
   figure(3);
   plot(u_sl);
   xlabel('$\sigma$','Interpreter','latex');
   ...
   in_int_term1 = chebfun2(@(lambda, sigma) sum(j0(omega*sigma)*j1(omega*xi)...
   *cosine(omega*lambda)), [0 upL 0.001 upS], 'vectorize');
   % eta(sigma, lambda)
   eta_sl = chebfun2(@(sigma,lambda) 1/4*sum(xi.^2*Phi(xi)*...
   in_int_term1(sigma, lambda))-1/2*u_sl(sigma, lambda).^2,...
