...
t0 = 0; %initial time
Tf = 5; %final time (10 in Catalina 1 experiement)
...
u_0 = @(x) -2.*exp(-(x-4).^2).*(sqrt(x+eta_0(x))-sqrt(x));
u_prime = chebfun(@(x) -0.01*2*(x-3)*exp(-(x-3).^2), [x0 Xf]);

%Nicolsky 2018 solution
%[ana_eta, ana_u] = CG_transform(true, 100, true, false);
...
%Plot_xt(ana_eta, 500, 2, 'Eta Anaylytic', 'x', 't', 'eta')
% Plot_xt(eta_diff, 500, 3, 'Eta Diff', 'x', 't', 'diff')
%
Plot_xt(num_u, 500, 3, 'U Numerical Solution (Deny FV)', 'x', 't', 'u')
%Plot_xt(ana_u, 500, 4, 'U Anaylytic', 'x', 't', 'u')
%Plot_xt(diff, 500, 5, 'U Diff', 'x', 't', 'diff')
