...
function [eta_sl, u_sl] = catalina1(save)
    ...
    eta_sigma_prime_0 = chebfun(@(sigma) H1*exp( -(c1*(sigma^2 - sigma1^2).^2)/256 ), [0 Ls]);%FIXME
    ...
    disp('j1...')
    j1 = chebfun(@(kx) besselj(1.0, kx), [0 Lo*Ls]);
    ...
    u_sl = chebfun2( @(sigma, lambda) sum( xi^2*Phi(sigma) * inner_e_sl_2(lambda, sigma)), [0.01 Ls 0 Ll], 'vectorize');

    
    disp('1...');

    inner_e_sl_1 = chebfun2(@(lambda, sigma) sum( j0(omega*sigma) * j1(omega*xi) * Cos(omega*lambda) ), [0 Ll 0.01 Ls], 'vectorize');


    disp('3...');

    eta_sl_1 = chebfun2( @(sigma, lambda) sum( xi^2*Phi(sigma) * inner_e_sl_1(lambda, sigma)), [0.01 Ls 0 Ll], 'vectorize');

    eta_sl = @(sigma,lambda) -1/4*eta_sl_1(sigma, lambda) -1/2*u_sl(sigma, lambda)^2;
